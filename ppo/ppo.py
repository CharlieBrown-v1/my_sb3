...
class HrlPPO(PPO):
    def __init__(
        ...
        # DIY
        self.success_rate_threshold = 0.7
        self.lower_rollout_buffer = None
        self.upper_rollout_buffer = None

        if _init_setup_model:
            self._setup_model()

    def net_train(self, indicate: int) -> None:
        assert indicate in indicate_list
        self.policy.set_training_mode(True)
        self._update_learning_rate(self.policy.optimizer)
        clip_range = self.clip_range(self._current_progress_remaining)
        if self.clip_range_vf is not None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                if indicate == lower:
                    values, log_prob, entropy = self.policy.lower_evaluate_actions(rollout_data.observations, actions)
                elif indicate == upper:
                    values, log_prob, entropy = self.policy.upper_evaluate_actions(rollout_data.observations, actions)
                else:
    ...
    def train_lower(self):
        self.net_train(indicate=lower)

    def train_upper(self):
        self.net_train(indicate=upper)

    def learn_upper(self,
                    callback: MaybeCallback = None,
                    log_interval: int = 1,
                    save_interval: Optional[int] = None,
                    save_path: Optional[str] = None,
        ...
        while iteration < train_estimate_iteration + train_upper_iteration:
            prefix = ''
            if iteration < train_estimate_iteration:
                prefix += 'estimate'
                continue_training = self.collect_rollouts(self.env, callback, self.lower_rollout_buffer,
                                                          n_rollout_steps=self.n_steps)
            else:
                prefix += 'upper'
                continue_training = self.collect_rollouts(self.upper_env, callback, self.upper_rollout_buffer,
                                                          n_rollout_steps=self.n_steps)
    ...
    def learn(
            ...
            save_path: Optional[str] = None,
            save_count: int = 0,
            train_estimate_interval: int = 100,  # every $train_estimate_interval iteration train once
            train_estimate_iteration: int = 50,
            train_upper_iteration: int = 3000,
    ) -> "HrlPPO":
        iteration = 0
        ...
        while self.num_timesteps < total_timesteps:
            ...
            if log_interval is not None and iteration % log_interval == 0:
                fps = int(self.num_timesteps / (time.time() - self.start_time))
                self.logger.record(f"lower/time/iterations", iteration, exclude="tensorboard")
                if len(self.ep_info_buffer) > 0 and len(self.ep_info_buffer[0]) > 0:
                    self.logger.record(f"lower/rollout/ep_rew_mean",
                                       safe_mean([ep_info["r"] for ep_info in self.ep_info_buffer]))
                    self.logger.record(f"lower/rollout/ep_len_mean",
                                       safe_mean([ep_info["l"] for ep_info in self.ep_info_buffer]))
                    if len(self.ep_success_buffer) > 0:
                        self.logger.record(f"lower/rollout/success_rate", safe_mean(self.ep_success_buffer))
                self.logger.record(f"lower/time/fps", fps)
                self.logger.record(f"lower/time/time_elapsed", int(time.time() - self.start_time), exclude="tensorboard")
                self.logger.record(f"lower/time/total_timesteps", self.num_timesteps, exclude="tensorboard")
            ...
            if save_interval is not None and iteration % save_interval == 0:
                ...
                save_count += 1
                self.save(save_path + "_" + str(save_count))
                self.logger.record(f"lower/Save Model", save_count)
                self.logger.record(f"lower/time/iterations", iteration)
                self.logger.record(f"lower/time/total_timesteps", self.num_timesteps)
                self.logger.dump(step=self.num_timesteps)

            if iteration % train_estimate_interval == 0:
                self.learn_upper(callback, log_interval, save_interval, save_path, save_count, train_estimate_iteration=train_estimate_iteration, train_upper_iteration=train_upper_iteration)
            else:
                self.train_lower()
    ...
    def _setup_model(self) -> None:
        ...
        self.lower_rollout_buffer = buffer_cls(
            self.n_steps,
            self.observation_space,
            self.action_space,
            self.device,
        ...
        self.upper_rollout_buffer = buffer_cls(
            self.n_steps,
            self.observation_space,
            self.action_space,
            self.device,
