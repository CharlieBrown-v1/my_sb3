...
class MultiInputActorCriticPolicy(ActorCriticPolicy):
    """
    MultiInputActorCritic policy class for actor-critic algorithms (has both policy and value prediction).
    Used by A2C, PPO and the likes.
...
class HybridPolicy(ActorCriticPolicy):
    ...
    Process:
        (length * width * height) -> NatureCNN -> pooling -> 64
        64 + physical_obs (25) = 89
    Used by A2C, PPO and the likes.
    ...
    def predict_observation(self, observation: Dict[str, np.ndarray]):
        ...
        value = self.value_net(latent_vf).flatten().item()

        return success_probability, value


# DIY
class AttnPolicy(HybridPolicy):
    """
    HybridClass policy class for actor-critic algorithms.
    Used by A2C, PPO and the likes.
    ...
    def __init__(
            ...
            use_expln: bool = False,
            squash_output: bool = False,
            features_extractor_class: Type[BaseFeaturesExtractor] = AttnExtractor,
            features_extractor_kwargs: Optional[Dict[str, Any]] = None,
            normalize_images: bool = True,
            optimizer_class: Type[th.optim.Optimizer] = th.optim.Adam,
            optimizer_kwargs: Optional[Dict[str, Any]] = None,
